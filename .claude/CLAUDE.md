...
- **Archive status bar at 600–899px**: `~/weebtrax/transmissions/` path hidden via `@media (min-width: 600px) and (max-width: 899px) { .wt-statusbar-path { display: none } }` in `styles.css` — landscape phone widths show only `094 files  sort: newest ↕` (no path).
...
- [x] TRACK I.D. toggle + ActiveRow layout refactor (2026-07-21) — `ActiveRow` returns a `React.Fragment` containing (1) the card div and (2) a tracklist sibling div rendered after the card. Tracklist opening no longer stretches the waveform box — the card height is fixed; the tracklist panel (`borderLeft/Right/Bottom: 1px solid WT2.line2`, `background: WT2.panel`) expands downward as a sibling. Card layout: CSS grid `oscColW px 1fr`, left col = oscilloscope only, right col = status + title + controls column (`flexDirection: column, alignItems: flex-start, gap: 8`); LISTEN button is first (natural content width, not stretched); secondary row (`wt-active-controls`) below contains YT/SC links, VolBar (non-mobile), TRACK I.D. toggle. Full-width seekbar is a third grid child with `gridColumn: '1 / -1'`. Old `wt-tracklist-break` flex-hack div and CSS `flex-basis`/`order` trick removed.
...
- [x] Footer social signals spacing (2026-07-21) — narrow branch (`winW < 900`) social signals grid now uses `gap: '10px 20px'` (was `'4px 10px'`) to match the full desktop layout exactly. Mobile (≤599px) unchanged (still gets 44px touch targets from `mobile.css`).
