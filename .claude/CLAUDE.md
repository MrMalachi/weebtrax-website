...
#### Still to do
- **The Railway database still holds the old `public/assets/…` paths.** It was seeded before this change, and the seed guard means a redeploy will *not* refresh it. It needs `python -m backend.init_db --reset` run against the Railway `DATABASE_URL` (or the tables cleared) before the deployed site will play anything.
- `production/public/assets/metadata/{mixes,scenes}.json` still carry the old prefixed paths and the 9 wrong filenames. Nothing reads them now that the frontend uses the API, but they're stale and should be regenerated or deleted to avoid being trusted later.
