...
- **Seeding made idempotent (2026-08-29)** — `add_sample_mixes`/`add_sample_scenes` now return early if their table already contains rows, and `backend/init_db.py` grew a `--reset` flag that truncates `track, mix, scene` with `RESTART IDENTITY CASCADE` before re-seeding. Previously the seed functions inserted unconditionally: running `python -m backend.init_db` a second time against the same database produced 198 mixes with no error or warning, surfacing only as a visibly doubled archive on the site. Confirmed by seeding a throwaway `weebtrax_seedtest` database twice before writing the fix, and confirmed fixed the same way afterwards (run 1 seeds, run 2 skips, `--reset` wipes and reseeds with ids back at 1–99).
  - The `RESTART IDENTITY` half matters as much as the guard: the frontend builds its `TX-` display codes from the primary key (`id 1` → `mix-001`), so a re-seed that left the sequence at 99 would renumber every mix on the site to `mix-100`+ while the underlying audio stayed the same.
  - This was flagged as a Phase 6 hazard specifically — a seed step wired into a Railway release command would have doubled the dataset on every single deploy.
